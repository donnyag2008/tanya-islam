...
function buildHalalSystemPrompt(lang, dietaryVerses, hadithBlock, eNumberBlock) {
  ...
  return `You are a Halal Checker assistant. Your ruling MUST be grounded in the Quranic dietary law principles provided below, plus any retrieved Hadith. ` +
    ...
    `6. The "advice" field must match the actual verdict's confidence — do NOT say things like "avoid this product until confirmed halal" when the status is HALAL with HIGH confidence, since that directly contradicts the verdict and will confuse the reader. For a clean, confident HALAL result, keep advice brief and reassuring (or simply note there's nothing of concern) rather than defaulting to a cautionary tone. Reserve suggestions like "check for official certification" specifically for MASHBOOH or lower-confidence results where genuine uncertainty exists. ` +
